...
@objc public protocol SwiftPhotoGalleryDataSource {
  ...
  func loadImage(forGallery gallery:SwiftPhotoGallery, at index: Int, complete:@escaping (UIImage?)->())
}
...
public class SwiftPhotoGallery: UIViewController {
  ...
  fileprivate var titleLabel: UILabel = UILabel()               // showing title
  ...
  private var needsLayout = true
    ...
    func updatePageControl() {
      ...
      let number = dataSource?.numberOfImagesInGallery(gallery: self) ?? 0
      titleLabel.text = "\(currentPage+1) of \(number)"
    }
  ...
  private func setupTitleLable() {
    ...
    titleLabel.frame = CGRect(x: 10, y: 20, width: 300, height: 30)
/*
    ...
    let yConstraint = NSLayoutConstraint(item: view,
  ...
  fileprivate func getImage(currentPage: Int, complete: @escaping (UIImage?)->()) {
    dataSource?.loadImage(forGallery: self, at: currentPage, complete: complete)
  }
